...
Describe 'Assert-MirrorTarget' {
    BeforeEach {
        $script:Mirror = Join-TestPath $TestDrive ('mirror-' + [guid]::NewGuid().ToString('N'))
    }
    ...
    It 'marks an empty folder with the source' {
        ...
        ([IO.File]::ReadAllText((Join-TestPath $script:Mirror '.restic-twin-mirror'))).Trim() | Should -Be 'C:\src'
    }

    It 'refuses a folder with files it did not create, and leaves them alone' {
        New-TextFile -Path (Join-TestPath $script:Mirror 'photos\2019.jpg') -Content 'precious'
        ...
        Join-TestPath $script:Mirror 'photos\2019.jpg' | Should -Exist
        Join-TestPath $script:Mirror '.restic-twin-mirror' | Should -Not -Exist
    }

    It 'refuses a mirror that belongs to another source' {
        New-TextFile -Path (Join-TestPath $script:Mirror '.restic-twin-mirror') -Content "C:\other`n"
    ...
    It 'accepts its own marker in another letter case' {
        New-TextFile -Path (Join-TestPath $script:Mirror '.restic-twin-mirror') -Content "c:\SRC`n"
        { Assert-MirrorTarget -MirrorPath $script:Mirror -SourcePath 'C:\src' } | Should -Not -Throw
    }
}

# The real mirror copy of this platform: robocopy on Windows, rsync on macOS.
Describe 'Update-Mirror' {
    BeforeAll {
        $source = Join-TestPath $TestDrive 'source'
        $destination = Join-TestPath $TestDrive 'dest'
        $config = New-TestSettingsFile -Folder (Join-TestPath $TestDrive 'cfg') -Source $source -Destination $destination
        [IO.File]::WriteAllText((Join-TestPath $TestDrive 'cfg\excludes-mirror.txt'), "secret.env`r`n")
        $script:Settings = Get-BackupSettings -ConfigPath $config
        New-TextFile -Path (Join-TestPath $source 'keep.txt') -Content 'v1'
        New-TextFile -Path (Join-TestPath $source 'app\node_modules\big.js') -Content 'rebuildable'
        New-TextFile -Path (Join-TestPath $source 'app\index.js') -Content 'code'
        New-TextFile -Path (Join-TestPath $source 'secret.env') -Content 'TOKEN=1'
        New-Item -ItemType Directory -Path $script:Settings.LogsPath, $script:Settings.MirrorPath -Force | Out-Null
        $script:FirstExit = Update-Mirror -Settings $script:Settings -LogPath (Join-TestPath $script:Settings.LogsPath 'mirror_1.log')
    }

    It 'copies the source with a success code' {
        $script:FirstExit | Should -BeLessThan 8
        Get-Content -LiteralPath (Join-TestPath $script:Settings.MirrorPath 'keep.txt') | Should -Be 'v1'
        Join-TestPath $script:Settings.MirrorPath 'app\index.js' | Should -Exist
    }

    It 'leaves out both exclude lists' {
        Join-TestPath $script:Settings.MirrorPath 'app\node_modules' | Should -Not -Exist
        Join-TestPath $script:Settings.MirrorPath 'secret.env' | Should -Not -Exist
    }

    It 'deletes what left the source but keeps its marker' {
        Remove-Item -LiteralPath (Join-TestPath $script:Settings.SourcePath 'keep.txt')
        $exit = Update-Mirror -Settings $script:Settings -LogPath (Join-TestPath $script:Settings.LogsPath 'mirror_2.log')
        $exit | Should -BeLessThan 8
        Join-TestPath $script:Settings.MirrorPath 'keep.txt' | Should -Not -Exist
        Join-TestPath $script:Settings.MirrorPath '.restic-twin-mirror' | Should -Exist
    }

    It 'writes a log of the copy' {
        Join-TestPath $script:Settings.LogsPath 'mirror_1.log' | Should -Exist
    }
}

Describe 'Invoke-HotCopySet' {
    BeforeEach {
        $script:Root = Join-TestPath $TestDrive ('hot-' + [guid]::NewGuid().ToString('N'))
        $script:A = Join-TestPath $script:Root 'src\board.html'
        $script:B = Join-TestPath $script:Root 'src\data\board.data.js'
        ...
        $script:Copies = Join-TestPath $script:Root 'hot-copies'
    ...
    It 'copies the whole set into one timestamped folder' {
        ...
        Get-Content -LiteralPath (Join-TestPath $script:Copies 'board\2026-09-23_100000\board.html') | Should -Be 'html v1'
        Get-Content -LiteralPath (Join-TestPath $script:Copies 'board\2026-09-23_100000\board.data.js') | Should -Be 'data v1'
    }
    ...
    It 'copies both files again when only one of them changed' {
        ...
        $r = Invoke-HotCopySet -Set $script:Set -HotCopiesPath $script:Copies -Now $script:T0.AddMinutes(5)
        $r.Result | Should -Be 'copied'
        Get-Content -LiteralPath (Join-TestPath $script:Copies "board\$($r.Version)\board.data.js") | Should -Be 'data v2'
        Join-TestPath $script:Copies "board\$($r.Version)\board.html" | Should -Exist
    }

    It 'keeps only the newest Keep versions and never touches other folders' {
        $other = Join-TestPath $script:Copies 'board\my-notes'
        New-TextFile -Path (Join-TestPath $other 'keep-me.txt')
        for ($i = 1; $i -le 5; $i++) {
        ...
        @(Get-HotCopyVersions -SetPath (Join-TestPath $script:Copies 'board') | ForEach-Object { $_.Name }) -join ',' | Should -Be '2026-09-23_100300,2026-09-23_100400,2026-09-23_100500'
        $r.Removed -join ',' | Should -Be '2026-09-23_100200'
        Join-TestPath $other 'keep-me.txt' | Should -Exist
    }
    ...
    It 'adds a suffix instead of overwriting a version from the same second, and sorts it numerically' {
        ...
        $names = @(Get-HotCopyVersions -SetPath (Join-TestPath $script:Copies 'board') | ForEach-Object { $_.Name })
        $names[0] | Should -Be '2026-09-23_100000'
        $names[-1] | Should -Be '2026-09-23_100000-11'
        Get-Content -LiteralPath (Join-TestPath $script:Copies 'board\2026-09-23_100000-11\board.html') | Should -Be 'same second 11'
    }
    ...
    It 'fails without writing a partial version when a file is missing' {
        ...
        @(Get-HotCopyVersions -SetPath (Join-TestPath $script:Copies 'board')).Count | Should -Be 0
    }

    It 'cleans a staging folder left by a crashed run' {
        New-TextFile -Path (Join-TestPath $script:Copies 'board\.incoming-dead\board.html')
        Invoke-HotCopySet -Set $script:Set -HotCopiesPath $script:Copies -Now $script:T0 | Out-Null
        Join-TestPath $script:Copies 'board\.incoming-dead' | Should -Not -Exist
    }
}

Describe 'hot-copy.ps1' {
    It 'copies each set, reports failures per set and exits 1 when one failed' {
        $source = Join-TestPath $TestDrive 'hc-source'
        $good = Join-TestPath $source 'notes.md'
        New-TextFile -Path $good -Content 'n'
        $extra = "    HotCopies = @(@{ Name = 'notes'; Files = @('$good') }, @{ Name = 'gone'; Files = @('$(Join-TestPath $source 'missing.txt')') })"
        $config = New-TestSettingsFile -Folder (Join-TestPath $TestDrive 'hc-cfg') -Source $source -Destination (Join-TestPath $TestDrive 'hc-dest') -Extra $extra
        ...
        Join-TestPath $TestDrive 'hc-dest\hot-copies' | Should -Not -Exist
        New-Item -ItemType Directory -Path (Join-TestPath $TestDrive 'hc-dest\hot-copies') -Force | Out-Null
        ...
        @(Get-HotCopyVersions -SetPath (Join-TestPath $TestDrive 'hc-dest\hot-copies\notes')).Count | Should -Be 1
        # Its log lives with the hot copies: logs belongs to the SYSTEM or root job.
        Get-Content -LiteralPath (Join-TestPath $TestDrive 'hc-dest\hot-copies\hot-copies.log') -Raw | Should -BeLike '*gone: error*'
        Join-TestPath $TestDrive 'hc-dest\logs\hot-copies.log' | Should -Not -Exist
    }
}

Describe 'Set-PrivateFolderAcl and Test-PrivateFolderAcl on Windows' -Skip:(-not $script:OnWindowsTest) {
    It 'makes a folder read-only for you and full control for SYSTEM and Administrators' {
        $folder = Join-TestPath $TestDrive 'acl-read'
        ...
        { [IO.File]::WriteAllText((Join-TestPath $folder 'x.txt'), 'x') } | Should -Not -Throw
    }
    ...
    It 'stops your account from writing there, from a shell that is not elevated' -Skip:(Test-TestElevated) {
        $folder = Join-TestPath $TestDrive 'acl-read-write'
        New-Item -ItemType Directory -Path $folder | Out-Null
        Set-PrivateFolderAcl -Path $folder -UserAccess Read
        try {
            { [IO.File]::WriteAllText((Join-TestPath $folder 'x.txt'), 'x') } | Should -Throw
...
# The root-owned, read-only side needs sudo: tests/ci-system-install-macos.ps1 covers it.
Describe 'Set-PrivateFolderAcl and Test-PrivateFolderAcl on macOS' -Skip:$script:OnWindowsTest {
    It 'makes a folder private to you: mode 700 and no ACL entry left from anyone' {
        $folder = Join-TestPath $TestDrive 'acl-private'
        New-Item -ItemType Directory -Path $folder | Out-Null
        ...
        Set-PrivateFolderAcl -Path $folder -UserAccess Full
        Test-PrivateFolderAcl -Path $folder -UserAccess Full | Should -BeTrue
        (@(& /usr/bin/stat -f '%Lp' $folder) -join '').Trim() | Should -Be '700'
        @(& /bin/ls -led $folder | Where-Object { $_ -match '^\s*\d+:\s' }).Count | Should -Be 0
    }
}

Describe 'Enter-RunLock' {
    It 'lets one holder in at a time, across processes' {
        $lock = Enter-RunLock -Name "test-$PID" -Folder $TestDrive
        try {
            $lock | Should -Not -BeNullOrEmpty
            $powershell = (Get-Process -Id $PID).Path
            $probe = ". '$([IO.Path]::Combine($script:RepoRoot, 'scripts', 'common.ps1'))'; if (`$null -eq (Enter-RunLock -Name 'test-$PID' -Folder '$TestDrive')) { exit 7 } else { exit 0 }"
        ...
        $again = Enter-RunLock -Name "test-$PID" -Folder $TestDrive
...
Describe 'Get-RunLockName' {
    ...
    It 'blocks a second run on the same destination and not a run on another one, across processes' {
        $here = @{ DestinationRoot = (Join-TestPath $TestDrive "lock-a-$PID") }
        $lock = Enter-RunLock -Name (Get-RunLockName -Kind backup -Settings $here) -Folder $TestDrive
        try {
            $lock | Should -Not -BeNullOrEmpty
            $powershell = (Get-Process -Id $PID).Path
            $common = [IO.Path]::Combine($script:RepoRoot, 'scripts', 'common.ps1')
            foreach ($case in @(@{ Destination = $here.DestinationRoot; Expected = 7 }, @{ Destination = (Join-TestPath $TestDrive "lock-b-$PID"); Expected = 0 })) {
                $probe = ". '$common'; `$l = Enter-RunLock -Name (Get-RunLockName -Kind backup -Settings @{ DestinationRoot = '$($case.Destination)' }) -Folder '$TestDrive'; if (`$null -eq `$l) { exit 7 }; Exit-RunLock `$l; exit 0"
...
Describe 'run-hidden.vbs' -Skip:(-not $script:OnWindowsTest) {
    BeforeAll {
        $script:Wscript = [IO.Path]::Combine($env:WINDIR, 'System32', 'wscript.exe')
        $script:Vbs = [IO.Path]::Combine($script:RepoRoot, 'scripts', 'run-hidden.vbs')
        $script:PowerShellPath = (Get-Process -Id $PID).Path
    }

    It 'returns the exit code of the program it ran, with arguments that contain spaces' {
        $marker = Join-TestPath $TestDrive 'folder with spaces\ran.txt'
    ...
    It 'exits 1 when the program cannot be started' {
